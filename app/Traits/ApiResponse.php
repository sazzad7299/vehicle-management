...
trait ApiResponse
{
    ...
    protected function respondDelete(string $message = ''): JsonResponse
    {
        return $this->apiResponse(
            [
                'message' => $message,
     ...
     * Respond with error.
     */
    protected function respondExpireLimit(
        int $statusCode = 400,
        int $error_code = 1,
        ?Throwable $exception = null
    ): JsonResponse {
        return $this->apiResponse(
            [
                'status' => ResponseAlias::HTTP_INTERNAL_SERVER_ERROR,
                'message' => 'Your Subscirbtion limit reached',
                'exception' => $exception,
                'error_code' => $error_code,
            ],
            $statusCode
    ...
    protected function respondExpired(
        string $message = 'Youre subscription plan expired. Renew Now'
    ): JsonResponse {
        return $this->respondError($message, 403);
    }
