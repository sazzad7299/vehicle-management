                            ...
                            <tbody>
                                <template v-if="customers && customers.length > 0">
                                    <tr :class="index % 2 === 0 ? 'odd' : 'even'" v-for="(customer, index) in customers">
                                        ...
                                        <td>
                                            {{ customer.sales_sum_total !== null ? customer.sales_sum_total : 0 }}
                                        </td>
                                        <td>
                                            {{ customer.sale_return_sum_price !== null ? customer.sale_return_sum_price : 0
                                            }}
                                        </td>
                                        <td>
                                            {{ customer.sale_payments_sum_paid !== null ? customer.sale_payments_sum_paid :
                                                0 }}
                                        </td>
                                        <td>
                                            {{ customer.sale_return_sum_return_amount !== null ?
                                                customer.sale_return_sum_return_amount : 0
                                            }}
                                        </td>
                                        <td>{{  (customer.sales_sum_total - customer.sale_payments_sum_paid) -
                                            (customer.sale_return_sum_price -
                                                customer.sale_return_sum_return_amount) }}</td>
                                    </tr>
                                    <tr>
...
export default {
    ...
    computed: {
        ...
        totalSumPurchase() {
            ...
            return this.customers.reduce((sum, customer) => sum + customer.sales_sum_total, 0);
        },
        totalSumPaid() {
            // Calculate the sum of total for all sales
            return this.customers.reduce((sum, customer) => sum + (customer.sale_payments_sum_paid ? parseInt(customer.sale_payments_sum_paid) : 0), 0);

        },
        totalSumDue() {
            // Calculate the sum of total for all sales
            return this.customers.reduce((sum, customer) => sum + (customer.sales_sum_total - customer.sale_payments_sum_paid) -
                                            (customer.sale_return_sum_price -
                                                customer.sale_return_sum_return_amount), 0);
