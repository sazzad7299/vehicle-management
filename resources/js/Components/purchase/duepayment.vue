<template>
    <div class="container-xxl flex-grow-1 container-p-y">
      <h4 class="fw-bold py-3 mb-4">
        <span class="text-muted fw-light"><router-link :to="{ name: 'sale.index' }">Purchase</router-link> /</span>Supplier Due
        collect
      </h4>
      <div class="card container-p-y">
        <h5 class="card-header fw-bold">Supplier Wise Due Collect</h5>
        <div class="card-body">
          <div class="row">
            <div class="col-md-6">
              <form @submit.prevent="dueCollect">
                <div class="mb-3 col-md-12">
                  <label class="form-label" for="basic-default-name">Select Supplier<span class="text-danger">*</span>
                  </label>
                  <v-select :options="customers" :label="'phone'" name="supplier_id" :placeholder="'Select Supplier'"
                    :reduce="(customer) => customer.id" v-model="form.supplier_id"></v-select>
                </div>
                ...
                <div class="mb-3 col-md-12" v-else>
                  <p>This Supplier no due available</p>
                </div>
              </form>
            </div>
            <div class="col-md-6">
              <div class="card-body border border-1 border-solid border-success" v-if="customer !== null">
                <div class="user-avatar-section">
                  <div class="d-flex align-items-center flex-column">
                    ...
                    <div class="user-info text-center">
                      ...
                      <span class="badge bg-label-secondary">Supplier <router-link
                          :to="{ name: 'supplier.view', params: { id: customer.id } }"
                ...
                <div class="d-flex justify-content-around flex-wrap my-4 py-3 text-center">
                  <div>
                    <h5 class="mb-0">
                      {{
                        customer.purchase_sum_total != null
                        ? customer.purchase_sum_total
                        : 0
                      }}
                    </h5>
                    <span>Purchase</span>
                  ...
                  <div>
                    <h5 class="mb-0">
                      {{
                        customer.purchase_payment_sum_paid != null
                        ? customer.purchase_payment_sum_paid
                        : 0
                      }}
                    </h5>
                    <span>Paid</span>
                      ...
                      {{
                        (customerDue =
                          ...
                          (customer.purchase_return_sum_price -
                            customer.purchase_return_sum_return_amount))
                      }}
                    </h5>
                    <span>Due</span>
                  </div>
  ...
  export default {
    ...
    watch: {
      "form.supplier_id": function (val, oldval) {
        if (val > 0) {
    ...
    methods: {
      ...
      getCustomerData() {
        axios
          .get("supplier/" + this.form.supplier_id)
          .then((response) => {
      ...
      dueCollect() {
        this.loader(true);
        axios
          .post("/supplier-due-payment", this.form)
          .then(response => {
            this.loader(false);
