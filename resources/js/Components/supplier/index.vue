<template>
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Supplier /</span>Supplier List</h4>
        <div class="card">
                ...
                <div class="dataTables_wrapper dt-bootstrap5 no-footer">
                    <div class="row ms-2 me-3">
                        <div
                            ...
                            <div class="dt-action-buttons text-xl-end text-lg-start text-md-end text-start mt-md-0 mt-3" v-if="hasPermission('supplier.create')">
                                    ...
                                    <router-link :to="{ name: 'supplier.create' }">
                                        <button class="btn btn-secondary btn-primary" type="button">
                                            <span>
                                                ...
                                                <span class="d-md-inline-block d-none">Create Supplier</span>
                                            </span>
                                        </button>
                                    </router-link>
                                </div>
                    ...
                    <table class="invoice-list-table table border-top dataTable no-footer dtr-column">
                        <thead>
                            <tr>
                                ...
                                <th>Email</th>
                                <th>Company</th>
                                <th v-if="hasPermission('supplier.view') || hasPermission('supplier.delete') || hasPermission('supplier.edit')">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr :class="index % 2 === 0 ? 'odd' : 'even'" v-for="(user, index) in users.data">
                                ...
                                <td class="d-flex px-1 justify-content-around" v-if="hasPermission('supplier.view') || hasPermission('supplier.delete') || hasPermission('supplier.edit')">
                                    <router-link :to="{ name: 'supplier.edit', params: { id: user.id } }" v-if="hasPermission('supplier.edit')">
                                        <button type="button" class="btn btn-sm btn-primary hide-arrow">
                                            <i class="bx bx-edit-alt"></i>
                                        </button>
                                    </router-link>
                                    <router-link :to="{ name: 'supplier.view', params: { id: user.id } }" v-if="hasPermission('supplier.view')">
                                        <button type="button" class="btn btn-sm btn-primary hide-arrow">
                                            <i class='bx bxs-bullseye'></i>
                                        </button>
                                    </router-link>
                                    <button type="button" class="btn btn-sm btn-danger hide-arrow"
                                        @click="softDeleteUser(user)" v-if="hasPermission('supplier.delete')">
