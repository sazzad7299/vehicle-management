<template>
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4">
            <span class="text-muted fw-light">Types /</span>Type List
        </h4>
        <div class="card">
                ...
                <div class="dataTables_wrapper dt-bootstrap5 no-footer">
                    <div class="row ms-2 me-3">
                        <div
                            ...
                            <div class="dt-action-buttons text-xl-end text-lg-start text-md-end text-start mt-md-0 mt-3" v-if="hasPermission('type.create')">
                                <div class="dt-buttons btn-group flex-wrap">
                                    <button class="btn btn-secondary btn-primary" type="button" @click="openAddModal">
                                        <span>
                                            <i class="bx bx-plus me-md-1"></i>
                                            <span class="d-md-inline-block d-none">Add Type</span>
                                        </span>
                                    </button>
                                </div>
                            </div>
                    ...
                    <table class="invoice-list-table table border-top dataTable no-footer dtr-column">
                        <thead>
                            <tr>
                                ...
                                <th>Note</th>
                                <th>Status</th>
                                <th v-if="hasPermission('type.delete') || hasPermission('type.edit')">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr :class="index % 2 === 0 ? 'odd' : 'even'" v-for="(unit, index) in units.data">
                                ...
                                <td class="d-flex py-2px-2" v-if="hasPermission('type.delete') || hasPermission('type.edit')">
                                    <div class="mx-2" v-if="hasPermission('type.edit')">
                                        <button type="button" class="btn btn-sm btn-primary hide-arrow"
                                            @click="openEditModal(unit)">
                                            <i class="bx bx-edit-alt mx-1"></i>
                                        </button>
                                    </div>
                                    <div v-if="hasPermission('type.delete')">
...
import UnitModal from "./typeModal.vue";
