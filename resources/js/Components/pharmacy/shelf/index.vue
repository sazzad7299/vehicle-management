<template>
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Shelf /</span>Shelf List</h4>
        <div class="card">
                ...
                <div class="dataTables_wrapper dt-bootstrap5 no-footer">
                    <div class="row ms-2 me-3">
                        <div
                            ...
                            <div class="dt-action-buttons text-xl-end text-lg-start text-md-end text-start mt-md-0 mt-3" v-if="hasPermission('shelf.create') ">
                                <div class="dt-buttons btn-group flex-wrap">
                                <button class="btn btn-secondary btn-primary" type="button"  @click="openAddModal">
                                    <span>
                                        <i class="bx bx-plus me-md-1"></i>
                                        <span class="d-md-inline-block d-none">Add Shelf</span>
                                    </span>
                                </button>
                                </div>
                            </div>
                    ...
                    <table class="invoice-list-table table border-top dataTable no-footer dtr-column">
                        <thead>
                            <tr>
                                ...
                                <th>Note</th>
                                <th>Status</th>
                                <th v-if="hasPermission('shelf.edit') || hasPermission('shelf.delete')">Action</th>
                        ...
                        <tbody>
                            <tr :class="index % 2 === 0 ? 'odd' : 'even'" v-for="(unit, index) in units.data">
                                ...
                                <td class="d-flex py-2px-2" v-if="hasPermission('shelf.edit') || hasPermission('shelf.delete')">
                                    <div class="mx-2" v-if="hasPermission('shelf.edit')">
                                        <button type="button" class="btn btn-sm btn-primary hide-arrow" @click="openEditModal(unit)">
                                            <i class="bx bx-edit-alt mx-1"></i>
                                        </button>
                                    </div>
                                    <div v-if="hasPermission('shelf.delete')">
                    ...
                    <div class="row mx-2">
                        <div class="col-sm-12 col-md-6">
                            <div class="fw-semibold" role="status" aria-live="polite">
                                Showing {{ units.from }}
                                to {{ units.to }} of {{ units.total }} entries
                            </div>
                        </div>
                        <div class="col-sm-12 col-md-6">
                            <pagination :data="units" :limit="10" :align="'right'"
                                @pagination-change-page="getPaginatedUnit($event)" />
                        </div>
                    </div>
                </div>
            </div>
    ...
    <unit-modal :is-open="isModalOpen" :mode="modalMode" :unit-data="modalUnitData" @submit="handleModalSubmit"
            @close="closeModal"  @unitAdded="this.getUnit({ page: 1 })"></unit-modal>
</template>
...
import UnitModal from './shelfModal.vue';
